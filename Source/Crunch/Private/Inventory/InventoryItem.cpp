...
void UInventoryItem::RemoveGASModifications(UAbilitySystemComponent* AbilitySystemComponent)
{
	...
	if (AppliedEquippedEffectHandle.IsValid()) AbilitySystemComponent->RemoveActiveGameplayEffect(AppliedEquippedEffectHandle);
	
	if (GrantedAbilitySpecHandle.IsValid()) AbilitySystemComponent->SetRemoveAbilityOnEnd(GrantedAbilitySpecHandle);
}
